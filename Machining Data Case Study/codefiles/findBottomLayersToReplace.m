function [learnableLayer,classLayer,softmaxProbLayer] = findBottomLayersToReplace(lgraph)
...
while true
    ...
    % return the class of the current layer and determine if it's a FC or
    % convolution layer
    currentLayerType = class(lgraph.Layers(currentLayerIdx));
    isLearnableLayer = ismember(currentLayerType, ...
        ['nnet.cnn.layer.FullyConnectedLayer','nnet.cnn.layer.Convolution2DLayer']);
    % if it is a learnable layer (FC, Conv), return true
    if isLearnableLayer
        learnableLayer =  lgraph.Layers(currentLayerIdx);
        return
    end
    % set the destination to the current layer and
